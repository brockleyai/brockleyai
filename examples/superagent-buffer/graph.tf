# E2E Superagent Buffer
#
# Terraform representation of the E2E Superagent Buffer test graph.
# See graph.json for the equivalent JSON representation.

variable "name" {
  type    = string
  default = "E2E Superagent Buffer"
}
...
resource "brockley_graph" "test" {
  ...
  [
    ...
    },
    {
      ...
      "config": {
        "prompt": "Reply with exactly: Report: test output",
        "provider": "openrouter",
