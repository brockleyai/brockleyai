...
#
# Terraform representation of the E2E Tool Loop Limits test graph.
# See graph.json for the equivalent JSON representation.

variable "name" {
  type    = string
  default = "E2E Tool Loop Limits"
}

resource "brockley_graph" "test" {
  name        = var.name
  namespace   = "e2e-tests"
  description = "E2E test: tool loop safety limit enforcement. LLM keeps requesting tool calls; should stop at limit."
