...
#
# Terraform representation of the E2E Tool Loop Error Recovery test graph.
# See graph.json for the equivalent JSON representation.

variable "name" {
  type    = string
  default = "E2E Tool Loop Error Recovery"
}

resource "brockley_graph" "test" {
  name        = var.name
  namespace   = "e2e-tests"
  description = "E2E test: behavior when a tool call fails and the LLM recovers."
  ...
  [
    ...
    },
    {
      ...
      "config": {
        "provider": "openai",
        "model": "tool-loop-error-recovery",
