# E2E Superagent Timeout
#
# Terraform representation of the E2E Superagent Timeout test graph.
# See graph.json for the equivalent JSON representation.

variable "name" {
  type    = string
  default = "E2E Superagent Timeout"
}
...
resource "brockley_graph" "test" {
  ...
  [
    ...
    },
    {
      ...
      "config": {
        "prompt": "Reply with exactly: timeout safe",
        "provider": "openrouter",
